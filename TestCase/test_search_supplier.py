...
class Test_alberta_git_search_dpt(softest.TestCase):
    ...
    @data(*Utilities.read_data_from_excel("C:\\Users\\getma\\PycharmProjects\\Alb\\TestData\\LoginData.xlsx","SupplierCode"))
    @unpack
    def test_searchven(self,code):
        self.lp.setUserName(self.User_Email)
        self.lp.setPassword(self.User_Password)
        self.lp.clickLogin()
        self.Vn.SrchModule()
        self.src.set_searchvendor(code)
        # self.src.display_Vendor_exist(code)
        time.sleep(2)
        self.driver.close()
